...
## 6. Business rules
- Priority: anyone reports low to high; only an admin can report a new ticket as critical (critical is shown site-wide). Afterwards an admin or an engineer on the ticket can change it to any level; the site alert clears by itself once a critical ticket is resolved. Changes always log a priority_changed event (old, new, reason).
