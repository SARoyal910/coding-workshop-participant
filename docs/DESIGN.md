...
## 6. Business rules
- Priority: anyone can report at any priority, critical included, so an emergency never waits for an admin; the form warns that critical alerts everyone on site. Afterwards an admin or an engineer on the ticket can change it to any level, and the site alert clears by itself once a critical ticket is resolved. Changes always log a priority_changed event (old, new, reason).
...
- Missed shift commitment = shift_ends_at < now AND not resolved by shift_ends_at AND not moved to blocked (with reason) before shift_ends_at. Computed at query time. Defined once (`_shared/engineer_stats.MISSED`) and used by both the counts and the Missed shift commitments page (`GET /api/dashboard/missed-shifts`), which lists each one with the engineer, the ticket, when they committed, when the shift ended, the ticket's status then and now, and when (if ever) it was resolved. Admins see everyone or one engineer; engineers see their own.
...
- Work logs: only engineers on the ticket; hours in 0.25 steps, 0.25–12; at most 12 hours per engineer per day across all tickets (checked under a per-engineer advisory lock so two simultaneous saves can't both pass); work_date not in future and not before created_at; editable by author only (logged); locked once archived.
